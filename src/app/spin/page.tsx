...
// Si mantienes el botón "Remove background (beta)" local, deja onnxruntime-web instalado.
// No lo usamos en este flujo cloud, pero no molesta tenerlo.
...
export default function SpinVideoPage() {
  ...
  const fileRef = useRef<File | null>(null); // guardamos el File original para /api/rvm
  ...
  // Estado del proceso en la nube (RVM)
  ...
  // === 1-click: subir a RVM (nube) → recibir video limpio (misma-origin) → extraer 36 frames
  const generateCleanCloud = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`RVM error: ${res.status}`);

      // Recibimos el video ya recortado como BLOB (mismo origen → no taint)
      const blob = await res.blob();
      const cleanUrl = URL.createObjectURL(blob);

      // Reemplazamos el src del video con el limpio y extraemos los frames
      if (objUrl) URL.revokeObjectURL(objUrl);
      setObjUrl(cleanUrl);
      setFrames([]); setCurrent(0); setLoaded(false); setDuration(0);

      // Esperamos metadatos del nuevo video
      ...
      console.error(e);
      alert("Fallo procesando en la nube. Verifica iluminación/encuadre o reintenta.");
    } finally {
      setCloudBusy(false);
    }
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-50 to-white text-slate-900 p-6">
      <div className="max-w-6xl mx-auto pt-10">
        ...
        <p className="mt-2 text-slate-600">
          Upload a short spin video. You can extract <b>{N_FRAMES} frames</b> locally or use <b>Cloud RVM</b> for cleaner background.
        </p>
          ...
          {objUrl && (
            <>
              ...
              <button
                ...
                title={!fileRef.current ? "Choose a video first" : "Upload to cloud, clean & extract"}
              >
              ...
              <div className="mt-3">
                <div className="h-2 bg-slate-200 rounded">
                  <div
                    ...
                    style={{ width: `${extracting ? progress : 100}%` }}
                  />
                </div>
